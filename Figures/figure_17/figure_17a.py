...
points_x_nosyst=np.load("points_x_k1_resum_nosyst.npy")
points_y_nosyst=np.load("points_y_k4_resum_nosyst.npy")
deltachisqs_all_nosyst=np.load("deltachisqs_all_resum_nosyst.npy")

points_x_syst=np.load("points_x_k1_resum_syst.npy")
points_y_syst=np.load("points_y_k4_resum_syst.npy")
...
plt.savefig("1_full_contour_plot_res_syst.pdf")
...
for i, collection in enumerate(CS.collections):
    ...
    if CS.levels[i]==5.991:
        ...
        for path in paths:
            ...
            ys2=vertices[:,1]
            plt.plot(xs2, ys2)
            plt.savefig("2_p=0.05_paths_res_syst.pdf")
...
for i, collection in enumerate(CS.collections):
    ...
    if CS.levels[i]==5.991:
        ...
        for path in paths:
            ...
            xs2=np.concatenate((vertices[45:3600,0], vertices[5600:9250,0]))#, vertices[3450:4250,0], vertices[4820:5800,0], vertices[6500:6850,0]))
            ys2=np.concatenate((vertices[45:3600,1], vertices[5600:9250,1]))#, vertices[3450:4250,1], vertices[4820:5800,1], vertices[6500:6850,1]))
...
plt.yticks(fontsize=22)
plt.legend(fontsize=20, loc="lower right")
plt.savefig("1_full_contour_plot_res_nosyst.pdf")
...
for i, collection in enumerate(CS.collections):
    ...
    if CS.levels[i]==5.991:
        ...
        for path in paths:
            ...
            ys1=vertices[:,1]
            plt.plot(xs1, ys1)
            plt.savefig("2_p=0.05_paths_res_nosyst.pdf")
...
for i, collection in enumerate(CS.collections):
    ...
    if CS.levels[i]==5.991:
        ...
        for path in paths:
            ...
            xs1=np.concatenate((vertices[0:2100,0], vertices[6800:7650,0], vertices[11250:14050,0]))#, vertices[3450:4250,0], vertices[4820:5800,0], vertices[6500:6850,0]))
            ys1=np.concatenate((vertices[0:2100,1], vertices[6800:7650,1], vertices[11250:14050,1]))#, vertices[3450:4250,1], vertices[4820:5800,1], vertices[6500:6850,1]))
            
            
            plt.plot(xs1, ys1, "r")
            plt.savefig("3_vertices_res_nosyst.pdf")
...
x_coord = np.linspace(-4,4,300)
y_coord = np.linspace(-4,4,300)
